...
Rails.application.routes.draw do
  refresh_list_routes = lambda { resource :refresh_list, only: [:create] }
  merge_list_routes = lambda { collection { resource :merge_lists, only: [:create] } }
  ...
  users_lists_routes = lambda { resources :users_lists, only: [:index, :create, :update, :destroy] }
  ...
  namespace :v2 do
    resources :lists, only: [:index, :show, :create, :edit, :update, :destroy] do
      ...
      resources :list_items, only: [:index, :show, :create, :edit, :update, :destroy] do
        bulk_update_routes[]
        resources :list_item_fields, only: [:index, :show, :create, :edit, :update, :destroy]
      end
      users_lists_routes[]
    end
  end
