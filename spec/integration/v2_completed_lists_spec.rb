...
describe "/v2/completed_lists", type: :request do
  ...
  describe "GET /" do
    it "responds with success and correct payload" do
      ...
      get v2_completed_lists_path, headers: auth_params

      response_body = JSON.parse(response.body).to_h
      expect(response).to have_http_status :success
      expect(response_body["completed_lists"].count).to eq user.all_completed_lists.count
